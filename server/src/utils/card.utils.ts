...
export class CardUtils {
  ...
  private static readonly RANKS: CardRank[] = [
    '7',
    '8',
    '9',
    'T',
  ...
  static canDefend(
    ...
    if (attackingCard.suit === trumpSuit && defendingCard.suit === trumpSuit) {
      return (
        this.getCardValue(defendingCard) > this.getCardValue(attackingCard)
      );
    }
  ...
  static canThrowCard(
    throwingCard: Card,
    activeCards: { attackingCard: Card; defendingCard?: Card }[],
  ): boolean {
    ...
    for (const cardPair of activeCards) {
      ...
      if (
        cardPair.defendingCard &&
        throwingCard.rank === cardPair.defendingCard.rank
      ) {
        return true;
      }
    }
